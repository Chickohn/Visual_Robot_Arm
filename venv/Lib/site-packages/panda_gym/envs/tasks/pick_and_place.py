...
class PickAndPlace(Task):
    ...
    def compute_reward(self, achieved_goal, desired_goal, info: Dict[str, Any]) -> np.ndarray:
        ...
        if self.current_subgoal == "above":
            if (d < (self.distance_threshold)).all() and self.gripper_width() > 0.06:
                reward = 1.0  # Reward for reaching the object
                self.subgoal_counters["above_complete"] += 1  # Increment counter
                print("Above complete count:", self.subgoal_counters["above_complete"])
        ...
        # Sub-goal 2: Grabbing the object
            ...
            if self._check_grabbed():
                reward = 5.0  # High reward for successful grabbing
                ...
                reward = -0.1 * d  # Continue to penalize based on distance to encourage correct positioning
        ...
        # Sub-goal 3: Moving the object to the target location
            ...
            if (od < self.distance_threshold*2).all():  # Assuming `d` is recalculated for object-to-goal distance
                ...
                reward = -od # -od  # Penalize heavily for dropping the beaker
    ...
    # def compute_reward(self, achieved_goal, desired_goal, info: Dict[str, Any]) -> np.ndarray:
    #     d = distance(achieved_goal, desired_goal)
    #     prev_distance = getattr(self, 'prev_distance_to_goal', None)
    #     self.prev_distance = d  # Update previous distance after use

    #     # General approach for movement penalty or reward
    #     print("d:", d, prev_distance)
    #     if prev_distance is not None:
    #         if (d < prev_distance).any():
    #             movement_reward = 0.1  # Reward for moving closer
    #         else:
    #             movement_reward = -0.1  # Penalty for moving away
    #     else:
    #         movement_reward = 0  # No reward or penalty on the first step

    #     # Initialize reward with movement reward
    #     reward = movement_reward
    #     reward = np.float32(reward)

    #     # Sub-goal 1: Reaching the object
    #     if self.current_subgoal == "above":
    #         # Reward for being in position with correct gripper width
    #         reward += np.where((d < self.distance_threshold * 2) & (self.gripper_width() > 0.06), 1.0, 0.0)
            
    #         # Check for premature grabbing across all environments
    #         grabbed = np.array([self._check_grabbed() for _ in range(len(d))])
    #         reward += np.where(grabbed, 5.0, 0.0)
            
    #         # Update subgoals based on conditions
    #         self.current_subgoal = np.where(grabbed, "lift", "pregrab")

    #     elif self.current_subgoal == "pregrab":
    #         if (d < self.distance_threshold).any():
    #             reward += 3.0  # Larger reward for reaching precise pre-grab location
    ...
    #             self.initial_ee_position = None
    #         else:
    #             reward += -0.1 * d  # Penalty for incorrect positioning

    #         if ee_movement > 0.01:
    #             reward -= 5 * ee_movement  # Severe penalty for any EE movement

    #     # Sub-goal 3: Moving the object to the target location
    #     elif self.current_subgoal == "lift":
    #         print("Lifting")
    #         od = distance(self.sim.get_base_position("base"), self.get_ee_position())
    #         if od < self.distance_threshold * 2 and d < self.distance_threshold:
    #             print("Lift complete")
    #             self.win = True
    #             reward = 10.0  # Highest reward for successful placement
    #         else:
    #             reward = -max(od, d)  # Penalty for incorrect placement or dropping

    #     # Additional penalty for incorrect beaker orientation
    #     beaker_orientation = abs(self.sim.get_base_orientation("object") - np.array([0.5, 0.5, 0.5, 0.5]))
    #     if any(beaker_orientation > np.array([0.1, 0.1, 0.1, 0.1])):
    #         reward -= sum(beaker_orientation)
